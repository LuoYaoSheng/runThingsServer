...
// 下发指令
func (on *OneNet) Cmds(id int, body interface{}) (bool, *string) {
	api := "/cmds?device_id=" + strconv.Itoa(id)
	return on.call(&api, ALLOW_METHODS["POST"], body, nil)
}
...
func (on *OneNet) DevicesStatus(ids string) (bool, *string) {
	api := "/devices/status?devIds=" + ids
	return on.call(&api, ALLOW_METHODS["GET"], nil, nil)
}
